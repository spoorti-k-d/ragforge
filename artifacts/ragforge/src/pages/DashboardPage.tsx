...
export default function DashboardPage() {
  ...
  const activityData = activity.map((a) => ({
    date: formatLocalTime(a.date, 'MMM d'),
    queries: a.count,
  }))
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8">
          ...
          <MetricCard
            ...
            icon={Layers}
            accent="cyan"
            sub={`${formatNumber(stats?.total_embeddings || 0)} emb.`}
          />
          <MetricCard
            label="Queries"
          ...
          <MetricCard
            ...
            icon={Clock}
            accent="amber"
            sub={`Conf: ${(stats?.avg_confidence_score || 0).toFixed(1)}%`}
          />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
        <div className="card lg:col-span-2">
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div>
              <h2 className="text-text-primary font-semibold text-sm md:text-base">Query Activity</h2>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={activityData} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
              ...
              />
              <Area type="monotone" dataKey="queries" stroke="#4F46E5" strokeWidth={2} fill="url(#queryGradient)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div>
              <h2 className="text-text-primary font-semibold text-sm md:text-base">Doc Types</h2>
              <p className="text-text-muted text-xs mt-0.5">By format</p>
            </div>
          ...
          {pieData.length > 0 ? (
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                ...
                <Tooltip contentStyle={{ background: '#1E293B', border: '1px solid #2D3F5E', borderRadius: 8, fontSize: 12 }} />
                <Legend formatter={(v) => <span style={{ color: '#94A3B8', fontSize: 11 }}>{v}</span>} />
              </PieChart>
            </ResponsiveContainer>
          ) : (
            <div className="flex items-center justify-center h-40 text-text-muted text-sm border-2 border-dashed border-bg-border rounded-xl">
              No documents yet
            </div>
          )}
        </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4 md:mb-5">
          <div>
            <h2 className="text-text-primary font-semibold text-sm md:text-base">Recent Queries</h2>
        ...
        {isLoading ? (
          ...
          <div className="py-8 md:py-10 text-center text-text-muted text-sm border-2 border-dashed border-bg-border rounded-xl">
            No queries yet — head to Ask AI to get started
          </div>
        ) : (
          <div className="divide-y divide-bg-border/50">
            {stats.recent_queries.map((q) => (
              <div key={q.id} className="py-3 md:py-3.5 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 hover:bg-bg-hover/40 transition-colors px-2 -mx-2 rounded-lg">
                <div className="flex-1 min-w-0">
                  <p className="text-text-primary text-sm font-medium truncate">{q.question}</p>
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    <p className="text-text-muted text-xs font-mono">
                      {formatLocalTime(q.created_at, 'MMM d, h:mm a')}
                    </p>
                    {q.used_reranker && (
                      <span className="text-brand-cyan bg-brand-cyan-dim px-1.5 py-0.5 rounded uppercase tracking-wider text-[9px] font-bold border border-brand-cyan/20">reranked</span>
                    )}
                  </div>
                </div>
